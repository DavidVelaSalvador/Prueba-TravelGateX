...
namespace Prueba_TravelGateX
{
    ...
    { 
            ...
            if (Received_Data.Trim() != "") /// esta condicion la pongo para evitar el procesamiento si no hemos tenido respuesta, evitando así errores.
            {
                try 
                {
                    ...
                    foreach (Atalaya_Meal_Plans_Type Temp_Meal_Plan in Meal_Plan_List_Received.meal_plans)  /// recorremos la lista de regímenes recibidos.
                    {
                        ...
                        {
                            Own_Format_Class Temp_Own_Data = new Own_Format_Class();       /// me creo un objeto para almacenar los datos
                            Process_Room(Temp_Own_Data,"ave", Temp_Room_Detail, Convert.ToString(Temp_Meal_Plan.code)); /// enviamos los datos a la funcion para que rellene los campos correspondientes.
                            Oficial_List.Add(Temp_Own_Data); /// Añadimos a la lista definitiva.
                        }
                                                                                                                        ...
                                                                                                                        /// el objeto con el modelo de datos nuestro propio
        {
            ...
            Room.rooms.price = Convert.ToDouble(Received_Details.price);  /// guardamos el precio convirtiendolo en un dato de tipo double.
        }
    }
}
